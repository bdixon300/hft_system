...
class Order
{
    public:
        ...
        Order(OrderId order, Price price, Side side, Quantity quantity, OrderType orderType):
            m_orderId(order),
            m_price(price),
            m_side(side),
            m_initialQuantity(quantity),
        ...
        void Fill(Quantity quantity)
        {
            ...
            {
                throw std::logic_error("Cannot fill an order quantity larger than remaining quantity");
            }
...
public:
    ...
    bool isOrderFullyFillable(const OrderPointer& order)
    {
        ...
        {
            return m_asks.count(order->getPrice()) && order->getQuantity() == m_askLevelData[order->getPrice()].totalQuantity;
        }

        return m_bids.count(order->getPrice()) && order->getQuantity() == m_bidLevelData[order->getPrice()].totalQuantity;
    }
    ...
    Trades addOrder(const OrderPointer& order)
    {
        ...
        if (order->getOrderType() == OrderType::FILLORKILL && !isOrderFullyFillable(order))
        {
        ...
        if (order->getSide() == Side::BUY)
        {
            ...
            m_orders.insert({order->getOrderId(), OrderEntry{order, std::prev(bids.end())}});
        ...
        else
        {
            ...
            m_orders.insert({order->getOrderId(), OrderEntry{order, std::prev(asks.end())}});
    ...
    void cancelOrder(const OrderPointer& order)
    {
        ...
        {
            // level data update
            m_bidLevelData[order->getPrice()].totalQuantity -= order->getRemainingQuantity();
        ...
        {
            // level data update
            m_askLevelData[order->getPrice()].totalQuantity -= order->getRemainingQuantity();
...
private:
    ...
    // order tracker
    std::unordered_map<OrderId, OrderEntry> m_orders; // makes it more efficient to cancel orders
